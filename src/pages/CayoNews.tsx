...
export const CayoNews: React.FC = () => {
      ...
      <FlexColumn className="weazel__home">
        <FlexLayout className="weazel__header">
          ...
          <div className="TypeSelector">
            <div
              className={"El" + (serverType === 0 ? " Selected" : "")}
              onClick={() => setServerType(0)}
            >
              WL
            </div>
            <div
              className={"El" + (serverType === 1 ? " Selected" : "")}
              onClick={() => setServerType(1)}
            >
              FA
            </div>
          </div>
        ...
        >
          ...
          <FlexLayout className="last__actus__wrapper">
            {news.slice(0, 3).map((n) => {
              if (n.type === "text")
                return (
                  <div className="actus FadeIn">
                    <Link to={`/cayo-news/${n.id}`}>
                      <div className="actus__img">
                        <img alt="artiste-img" src={n.media} />
                      </div>
                      <FlexColumn>
                        <h2 className="actus__title">{n.title}</h2>
                        <span className="actus__data">
                          {dayjs(n.date).format("DD.MM.YYYY - HH:mm")},{" "}
                          {n.character.firstName} {n.character.lastName}
                        </span>
                      </FlexColumn>
                      <BsJournal />
                    </Link>
              ...
              if (n.type === "video")
                return (
                  <div className="videoContainer FadeIn">
                    <iframe
                      src={
