...
export const SelectedCayoNews: React.FC = () => {
  ...
  const fetchData = async (forceId?: string) => {
    const response = await api.get(`/cayo-news?id=${forceId ?? id}`);
    setSelectedNews(response.data?.data?.news ?? null);

    if (!allNews) {
      const response2 = await api.get(
        `/cayo-news/last?limit=100&serverType=${serverType}`
      );
      setAllNews(
        response2.data?.data?.news?.filter((e) => e.type !== "video") ?? []
      );
    }
  };
  ...
  useEffect(() => {
    fetchData().catch((e) => {
      console.log(e);
      enqueueSnackbar(
        "Une erreur est survenue pendant la récupération de cet article."
      );
    });
  }, [id]);
  ...
  return (
    <HeaderAndFooterLayout>
      <FlexColumn className="weazel__page">
        ...
        {selectedNews && selectedNews.type === "text" && (
          <FlexColumn className="article">
            ...
            <FlexColumn className="article__content">
              ...
              <h1 className="article__title">{selectedNews.title}</h1>
              <span className="article__data">
                {dayjs(selectedNews.date).format("DD.MM.YYYY - HH:mm")},{" "}
                {selectedNews.character.firstName}{" "}
                {selectedNews.character.lastName}
              </span>
        ...
        {selectedNews && selectedNews.type === "image" && (
          <FlexColumn className="article">
            <FlexColumn className="article__content">
              <div className="CustomContainer">
                <Link to={"/cayo-news"}>
